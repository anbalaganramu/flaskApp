pipeline {
    ...
    stages {
        stage('Clone and Build Docker Image on Slave') {
            steps {
                script {
                    sshagent(['flaskApp']) { // Use the SSH credential ID you configured
                        echo 'Cloning repository and building Docker image on Slave...'

                        // Step 1: Clone the repository using Docker's git clone capability
                        // We'll run a temporary container with git to do the clone
                        // This avoids needing git installed directly on the slave OS
                        sh """
                            ssh -o StrictHostKeyChecking=no ${SLAVE_SERVER} '
                                # Remove any old clone directory
                                rm -rf ${APP_NAME} 
                                # Run a container with git to clone the repo
                                docker run --rm -v \$(pwd):/workspace alpine/git clone ${GIT_REPO} ${APP_NAME}
                            '
                        """
